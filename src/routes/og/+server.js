...
function setTemplate(title, description, category, img) {
	...
	var template = `
	<div style="display: flex; flex-direction: column; justify-content: flex-end; width: 600px; height: 500px; background-color: #000; color: #fff;">>
		<div tw="relative flex flex-col w-full">
			<div tw="bg-red-500 absolute flex"></div>
			<img src="${img}" alt="" tw="absolute -bottom-42 object-cover w-full" width="600"/>
		</div>
		...
		<div tw="flex justify-between h-28 w-full items-start p-3" style="background-color: ${categoryColor[4]};">
			<div tw="flex flex-col">
				<div tw="text-3xl flex" style="color: ${categoryColor[0]}">${title}</div>
				<div tw="text-xl text-white flex">${description}</div>
			</div>
			<div tw="mx-3 flex h-full items-center text-5xl" style="font-family: "Caveat"; color: ${categoryColor[0]}">VitroidFPV</div>
		</div>
	</div>
	`
