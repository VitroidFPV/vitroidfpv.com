...
function setTemplate(title: string, description: string, category: string, img: string) {
	switch (category) {
		case "Guides":
	...
	<div style="display: flex; flex-direction: column; justify-content: flex-end; width: ${width}; height: ${height}; background-color: #000; color: #fff;">>
		...
		<svg tw="h-48 max-w-full m-0 flex" id="visual" viewBox="0 0 2400 300" width="100%" height="300" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
		...
		<div tw="flex justify-between max-h-64 h-fit w-full items-start p-8" style="background-color: ${categoryColor[4]};">
			<div tw="flex flex-col">
				<div tw="text-6xl flex max-w-3xl" style="color: ${categoryColor[0]}">${title}</div>
...
const ReemKufiFont = await fetch('https://vitroidfpv.com/uploads/fonts/ReemKufi-Regular.ttf');
const ReemKufi = await ReemKufiFont.arrayBuffer();
const CaveatFont = await fetch('https://vitroidfpv.com/uploads/fonts/Caveat-Bold.ttf');
