<script>
    ...
    function copyVolume() {
        ...
        domtoimage.toPng(node, {height: 96, width: 400, bgcolor: "#0f0f0f"})
            ...
            <div>
                <div class="flex my-2 h-fit">
                    <div class="flex items-end">
                        <p>Motor Size:</p>
                        <input bind:value={size} on:input={calculateVolume} type="text" class="bg-gray-500/30 w-16 h-8 ml-2 rounded-md p-2 text-base duration-300
                        outline-none focus-within:outline-highlight outline-[3px]">
                    </div>
                    <div class="flex items-end ml-4">
                        <p>Result:</p>
                        <div class:invisible={isNaN(calculateVolume(size)) || calculateVolume(size) == 0} class="bg-gray-500/30 flex items-center align-middle min-w-[2rem] h-8 ml-2 rounded-md p-2 text-base text-green w-fit duration-300">{calculateVolume(size)}</div>
                        <p class:invisible={isNaN(calculateVolume(size)) || calculateVolume(size) == 0}>mm³</p>
                    </div>
                </div>
                <p class="">The motor stator size is listed as <code>xxyy</code>, with the <code>xx</code> being the width, and <code>yy</code> being the height. A 2207 motor will have a stator 22mm wide, and 7mm high<br><br>
                    The calculation to get the stator volume is for a simple cylinder: <code>π × r<sup>2</sup> × h</code>
                </p>
                <div class="flex mt-2">
                    <div class="h-fit w-fit bg-green/90 rounded-md py-1 px-2 cursor-pointer" on:click={copyVolume}>Copy!</div>
