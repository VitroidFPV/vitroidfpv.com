<script lang="ts">
	...
	let { buildSlug }: { buildSlug: string } = $props()

	const buildSection = $derived.by(() => {
		const section = getBuildSectionByUrl(`/builds/${buildSlug}`)

		if (!section) {
			throw new Error(`No build metadata found for "${buildSlug}"`)
		}

		return section
	})
	const BuildIntro = $derived(buildSection.component)
	const guideSections = $derived(getBuildGuideSections(buildSlug))
	...
	const listStorageKey = $derived(`build-guide-list:${buildSlug}`)
...
<PageWrapper
	h1={buildSection.title}
	h2={buildSection.description}
	seoDescription={buildSection.seoDescription}
